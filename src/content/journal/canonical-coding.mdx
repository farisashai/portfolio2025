...
For example:
- **Good convention**: "No redundant wrapper divs" — the wrapper provides no value and adds noise
- **Good convention**: "No inline types for shared shapes" — inline types create duplication and refactoring hazards
- **Bad convention**: "All forms must use library X" — if library X can't handle a legitimate product requirement, the convention blocks progress

The rule: **if a convention prevents solving a real problem, it's not a convention—it's a constraint**. Canonical codebases eliminate objectively worse patterns, not legitimate architectural choices.
